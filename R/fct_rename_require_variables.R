...
rename_require_variables <- function(
  ...
  if (is.null(megaplot_data)) { return(NULL)}

  #check if variable names are available in data and variable names are not empty
  if (
    !is.null(selected_subjectid) &
    !is.null(selected_event) & selected_event %in% colnames(megaplot_data) &
    (!is.null(selected_start_time) & !is.null(selected_end_time) #&
    ...
    if (!is.null(selected_event_time) & !is.null(selected_event_time_end)) {
      if (is.numeric(megaplot_data[[selected_event_time]]) & is.numeric(megaplot_data[[selected_event_time_end]])) {
        megaplot_data <- megaplot_data |>
          dplyr::mutate(
