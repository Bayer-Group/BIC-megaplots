...
draw_mega_plot <- function(
  ...
  if (sequencing_switch) {
    if (!is.null(sequencing_object)) {
      ...
      sequencing_ranks_data <- data.frame(
        megaplots_selected_subjectid = as.numeric(names(seriation::get_rank(
          sequencing_object
        ))),
      ...
      subjectids_without_sequencing_rank <- unique(
        megaplot_prepared_data$megaplots_selected_subjectid
      )[
        !unique(megaplot_prepared_data$megaplots_selected_subjectid) %in%
          sequencing_ranks_data$megaplots_selected_subjectid
      ]
      ...
      if (length(subjectids_without_sequencing_rank) > 0) {
        sequencing_unranked_data <- data.frame(
          ...
          SEQUENCING = (nrow(sequencing_ranks_data) + 1):(nrow(
            sequencing_ranks_data
          ) +
            length(subjectids_without_sequencing_rank))
        )

        #add rows of missing identifier
        if (nrow(sequencing_unranked_data) > 0) {
          sequencing_ranks_data <- rbind(
            sequencing_ranks_data,
            sequencing_unranked_data
          )
        }
      }
      ...
      sequencing_grouped_data <- megaplot_prepared_data_w_ranks |>
        ...
        dplyr::select(
          .data$megaplots_selected_subjectid,
          .data$SEQUENCING,
          .data$group_index,
          n_subjects
        ) |>
        dplyr::mutate(
          SEQUENCING_grouped = .data$SEQUENCING +
            (.data$group_index - 1) * n_subjects
        ) |>
        dplyr::distinct() |>
        dplyr::mutate(
          SEQUENCING_grouped_ranked = rank(.data$SEQUENCING_grouped)
        ) |>
        dplyr::select(
          .data$megaplots_selected_subjectid,
          .data$SEQUENCING_grouped_ranked
        ) |>
        dplyr::ungroup()
      ...
      megaplot_prepared_data <- megaplot_prepared_data |>
        ...
        dplyr::mutate(
          subjectid_n = .data$SEQUENCING_grouped_ranked +
            (.data$group_index - 1) * 10
        )
      ...
      megaplot_filtered_data <- megaplot_filtered_data |>
        ...
        dplyr::mutate(
          subjectid_n = .data$SEQUENCING_grouped_ranked +
            (.data$group_index - 1) * 10,
          subjectid_n_jittered = (.data$SEQUENCING_grouped_ranked +
            (.data$group_index - 1) * 10) +
            .data$jitter_event_time
        )
    }
  }

  min_start_day <- min(
    megaplot_prepared_data$megaplots_selected_event_time,
    na.rm = TRUE
  )
  max_end_day <- max(
    megaplot_prepared_data$megaplots_selected_event_time_end,
    na.rm = TRUE
  )
    ...
    megaplot_filtered_data <- megaplot_filtered_data |>
      dplyr::mutate(
        text_events = paste0(
          ...
          ") \n",
          " Start time: ",
          .data$megaplots_selected_event_time,
          "\n End time: ",
          .data$megaplots_selected_event_time_end
        )
      )

    #re-arrangement for plotly legend
    megaplot_filtered_data$event_group <- factor(
      megaplot_filtered_data$megaplots_selected_event_group,
      levels = sort_event_groups
    )

    megaplot_filtered_data <- megaplot_filtered_data |>
      dplyr::arrange(.data$event_group, .data$event_id)

    megaplot_filtered_data$unique_event <- factor(
      megaplot_filtered_data$unique_event,
      levels = unique(megaplot_filtered_data$unique_event)
    )
  }
  ...
  if (circular_vision) {
    ...
    megaplot_prepared_data <- megaplot_prepared_data |>
      dplyr::mutate(
        megaplots_selected_start_time = megaplot_prepared_data$megaplots_selected_start_time *
          cos(2 * pi * time), #x
        megaplots_selected_end_time = megaplot_prepared_data$megaplots_selected_end_time *
          cos(2 * pi * time), #xend
        subjectid_n = megaplot_prepared_data$megaplots_selected_start_time *
          sin(2 * pi * time), #y
        subjectid_n_end = megaplot_prepared_data$megaplots_selected_end_time *
          sin(2 * pi * time), #yend
      )
    time2 <- megaplot_filtered_data$subjectid_n / (number_subjects)
    megaplot_filtered_data <- megaplot_filtered_data |>
      dplyr::mutate(
        megaplots_selected_event_time = (megaplot_filtered_data$megaplots_selected_event_time -
          0.45) *
          cos(2 * pi * time2), # x
        megaplots_selected_event_time_end = (megaplot_filtered_data$megaplots_selected_event_time_end +
          0.45) *
          cos(2 * pi * time2), #xend
        subjectid_n_jittered = c(
          (megaplot_filtered_data$megaplots_selected_event_time - 0.45) *
            sin(2 * pi * time2)
        ), #y
        subjectid_n_jittered_end = c(
          (megaplot_filtered_data$megaplots_selected_event_time_end + 0.45) *
            sin(2 * pi * time2)
        ) #yend
      )
  } else {
    ...
    if (!is.null(megaplot_filtered_data)) {
      megaplot_filtered_data <- megaplot_filtered_data |>
        dplyr::mutate(
          ...
          megaplots_selected_event_time = .data$megaplots_selected_event_time -
            0.45,
          megaplots_selected_event_time_end = .data$megaplots_selected_event_time_end +
            0.45
        )
    }
  }
  ...
  if (
    !(all(is.na(megaplot_prepared_data$megaplots_selected_start_time)) |
      all(is.na(megaplot_prepared_data$megaplots_selected_end_time)))
  ) {
    p_1 <- p_1 |>
      plotly::add_segments(
        # create subject lines via add_segments
        y = ~subjectid_n,
        ...
        line = list(
          color = ifelse(theme == "dark", line_color_dark, line_color_light),
          width = line_width_subjects
        ),
        showlegend = FALSE
      )
  }
  ...
  if (!is.null(megaplot_filtered_data)) {
    if (event_tooltips) {
      ...
      p_2 <- p_1 |>
        plotly::add_segments(
          data = plotly::highlight_key(
            megaplot_filtered_data |> dplyr::filter(is.na(.data$n_flag)),
  ...
  p_3 <- p_2 |>
    plotly::config(
      ...
      #Remove not needed buttons from modebar
