...
sequencing_ui <- function(id) {
  ...
  bslib::accordion_panel(
    ...
    shinyWidgets::pickerInput(
      ...
      choices = sort(
        c(
          'GW_average', 'GW_complete', 'GW_single', 'GW_ward',
          'HC_single', 'HC_average', 'HC_complete', 'HC_ward',
          'OLO_average', 'OLO_complete', 'OLO_single', 'OLO_ward',
          'VAT', 'TSP', 'ARSA'
        )
      ),
      selected = 'GW_average',
    ...
    shinyWidgets::pickerInput(
      ...
      choices = c(
        "OM", "OMloc", "OMslen", "OMspell", "OMstran",
    ...
    bslib::accordion_panel(
      ...
      shiny::conditionalPanel(
        ...
        shinyWidgets::pickerInput(
          ...
          choices = sort(c("CONSTANT", "INDELS", "INDELSLOG", "TRATE", "ORDINAL")),
          selected = 'CONSTANT',
          multiple = FALSE,
          options = list(`live-search` = TRUE, `header` = 'Select item')
        )
      ...
      shiny::conditionalPanel(
        ...
        shinyWidgets::pickerInput(
          ...
          label = "Normalization",
          choices = sort(c("auto", "none", "maxlength", "gmean", "maxdist", "YujianBo")),
          selected = 'auto',
          multiple = FALSE,
          options = list(`live-search` = TRUE, `header` = 'Select item')
        )
...
sequencing_server <- function(input, output, session, megaplot_filtered_data) {
  ...
  shiny::observeEvent(input$sequencing_distmeasure_name, {
    if (input$sequencing_distmeasure_name == "DHD") {
      ...
      choices_sub_cost <- sort(c("CONSTANT", "INDELS", "INDELSLOG", "TRATE", "ORDINAL"))
    ...
    if (input$sequencing_distmeasure_name %in% c("CHI2", "EUCLID")) {
      ...
      selected_norm <- "auto"
    } else if (input$sequencing_distmeasure_name %in% c('OM', 'OMloc', 'OMslen', 'OMspell', 'OMstran', 'DHD', 'LCS', 'LCP', 'RLCP')) {
      choices_norm <- sort(c("auto", "none", "maxlength", "gmean", "maxdist", "YujianBo"))
      selected_norm <- "auto"
    }

    if (input$sequencing_distmeasure_name %in% c("OM", "OMloc", "OMslen", "OMspell", "OMstran", "DHD", "LCS", "LCP", "RLCP", "CHI2", "EUCLID")) {
      shinyWidgets::updatePickerInput(
        session,
  ...
  shiny::observeEvent(input$sequencing_button, {
    ...
    shiny::withProgress(message = "Apply seriation", value = 0, {
      for (i in seq_along(selected_event_for_sequencing)) {
        ...
        if (par$distmeasure == "OMloc" && any(apply(dist_init[, -1], 1, function(x) all(is.na(x))))) {
        ...
        dist_list[[i]] <- suppressMessages(do.call(TraMineR::seqdist, seqargs_all))
      }
    })
