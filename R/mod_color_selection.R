  ...
  bslib::nav_panel(
    ...
    shiny::fluidRow(
      ...
      shiny::column(
        4,
        shiny::wellPanel(
          id = ns("selected_events_color_container_panel"),
      ...
      shiny::column(
        ...
        shiny::conditionalPanel(
          condition = paste0("output['", ns("color_changed"), "'] == true"),
...
mod_color_selection_server <- function(
  ...
  shiny::moduleServer(id, function(input, output, session) {
    ...
    shiny::observe({
      ...
      if (
        is.na(color_data$selected[js_column$number, "megaplots_selected_event"])
      ) {
    ...
    shiny::observeEvent(
      c(input$update_color_palette, input$update_color_palette_2),
      {
    ...
    shiny::observe({
      ...
      shinyjs::runjs(
        sprintf(
          paste0(
            "document.getElementById('selected-cols-row').style.maxHeight =
               '",
            window_height() - 300,
            "px';"
          )
    ...
    shiny::observeEvent(
      ...
      ),
      {
        ...
        selected_data <- data.frame(
          megaplots_selected_event_group = NULL,
          megaplots_selected_event = NULL
        )
            ...
            for (i in seq_along(checked_tree)) {
              for (j in seq_along(checked_tree[[i]]$children)) {
                ...
                selected_data <- rbind(
                  ...
                  data.frame(
                    megaplots_selected_event_group = checked_tree[[i]]$text,
                    megaplots_selected_event = checked_tree[[i]]$children[[
                      j
                    ]]$text
                  )
                )
    ...
    shiny::observeEvent(
      js_column$number,
      {
        if (!is.null(js_column$number)) {} else {
          shinyjs::hideElement(id = "colour_picker_panel_1")
          shinyjs::hideElement(id = "colour_picker_panel_2")
          shinyjs::hideElement(id = "colour_picker_panel_3")
          shinyjs::hideElement(id = "update_color_palette")
    ...
    shiny::observeEvent(input$jitter_events, {
      ...
      if (!is.null(color_data$all)) {
        if (
          !is.na(color_data$selected[
            js_column$number,
            c("megaplots_selected_event")
          ])
        ) {
          color_data$all[
            color_data$all$megaplots_selected_event_group ==
          ...
          if (
            ...
            if (input$color_method == "gradient") {
              ...
              new_event_group_color2 <- new_event_group_color |>
                ...
                dplyr::mutate(
                  event_color = f_col_z(seq(
                    0,
                    1,
                    length = sum(!is.na(new_event_group_color$event_id))
                  ))[.data$event_id]
                ) |>
              ...
              selected_color_palette <- create_palette(
                n = number_events,
                name = input$select_color_palette
              )
    ...
    output$selected_events_color_container <- renderUI({
      if (!is.null(checked_data$val)) {
        if (nrow(checked_data$val) > 0) {
          ...
          lapply(
            seq_along(selected_data$names_for_color_list),
              ...
              div(
                ...
                div(
                  ...
                  style = paste0(
                    "border: ",
                    ifelse(
                      column_number == js_column$number,
                      "3px solid",
                      "0px"
                    ),
                    ";",
                    ...
                    "background:",
                    ifelse(
                      selected_data[column_number, ]$type ==
                        "megaplots_selected_event",
                      selected_data[column_number, ]$event_color,
      ...
      if (!is.null(color_data$selected) && !is.null(js_column$number)) {
        if (input$color_method == "gradient") {
          ...
          graphics::image(
            ...
            #grDevices::rgb(
            col = f_col_z(seq(0, 1, length = number_events)),
            #,maxColorValue = 255
            #),
            xlab = "",
            ylab = "",
    ...
    shiny::observeEvent(input$upload_saved_color_file, {
      ...
      if (dim(uploaded_colors)[1] == dim(color_data$all)[1]) {
        if (
          all(
            sort(unique(uploaded_colors$megaplots_selected_event)) ==
              sort(unique(color_data$all$megaplots_selected_event))
          )
        ) {
          uploaded_colors <- uploaded_colors |>
            dplyr::mutate(
              gradient_event_color_1 = dplyr::case_when(
                is.na(.data$megaplots_selected_event) ~ gradient_event_color_1,
                !is.na(.data$megaplots_selected_event) ~ NA
              ),
              gradient_event_color_2 = dplyr::case_when(
                is.na(.data$megaplots_selected_event) ~ gradient_event_color_2,
                !is.na(.data$megaplots_selected_event) ~ NA
              ),
              gradient_event_color_3 = dplyr::case_when(
                is.na(.data$megaplots_selected_event) ~ gradient_event_color_3,
                !is.na(.data$megaplots_selected_event) ~ NA
              )
            )
