...
create_unique_event_identifier <- function(
  ...
  megaplot_data_splitted_by_event_group <- split(
    megaplot_data_raw,
    megaplot_data_raw$megaplots_selected_event_group
  )
  ...
  if (length(megaplot_data_splitted_by_event_group) > 0) {
    ...
    for (i in seq_along(megaplot_data_splitted_by_event_group)) {
      ...
      megaplot_data_splitted_by_event_group[[
        i
      ]] <- megaplot_data_splitted_by_event_group[[i]] |>
      ...
      megaplot_data_splitted_by_event_group[[
        ...
        dplyr::mutate(
          max_event_id = max(
            megaplot_data_splitted_by_event_group[[i]]$event_id
          )
        )
      megaplot_data_splitted_by_event_group[[
        i
        ...
        dplyr::mutate(
          event_n = suppressWarnings(as.numeric(.data$megaplots_selected_event))
        ) |>
        #only neccessary when implementing numeric events
        dplyr::mutate(
          n_flag = dplyr::case_when(
            .data$megaplots_selected_event == .data$event_n ~ TRUE
          )
        )
      ...
      megaplot_data_splitted_by_event_group[[
        ...
        dplyr::ungroup() |>
        dplyr::add_row(
          megaplots_selected_event_group = unique(
            megaplot_data_splitted_by_event_group[[
              i
            ]]$megaplots_selected_event_group
          ),
          ...
          event_id = 0,
          event_group_id = unique(
            megaplot_data_splitted_by_event_group[[i]]$event_group_id
          ),
          max_event_id = unique(
            megaplot_data_splitted_by_event_group[[i]]$max_event_id
          ),
        )
    }

    #bind all list entries rowwise to one data.frame
    megaplot_data_w_event_ids <- do.call(
      "rbind",
      megaplot_data_splitted_by_event_group
    )

    number_event_groups <- length(unique(
      megaplot_data_w_event_ids$megaplots_selected_event_group
    ))
    ...
    megaplot_event_data_w_color <- megaplot_data_w_event_ids |>
      dplyr::rowwise() |>
      dplyr::mutate(
        event_color = color_func(
          .data$event_id,
          .data$event_group_id,
          .data$max_event_id,
          number_event_groups = number_event_groups
        )
      )
    ...
    megaplot_event_data_w_color <- megaplot_event_data_w_color |>
      dplyr::mutate(
        ...
        gradient_event_color_1 = dplyr::case_when(
          .data$event_id == 0 ~ grDevices::colorRampPalette(
            c("white", .data$event_color),
            alpha = TRUE
          )(100)[50],
          .data$event_id != 0 ~ NA
        ),
