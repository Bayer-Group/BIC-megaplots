  ...
  #
   if(nrow(tree) > 0) {
    ...
    filtered_data <- filtered_data %>%
      ...
      dplyr::left_join(
        color_data %>%
          dplyr::select(tidyselect::all_of(c("event","event_group","event_color","jittered","event_id"))),
        by = c("event","event_group")
      )

    filtered_data_w_jitter <- filtered_data  %>%
      dplyr::ungroup() %>%
      dplyr::select(tidyselect::all_of(c("event", "event_group", "max_event_id", "event_group_id", "event_id", "jittered"))) %>%
      dplyr::distinct() %>%
    ...
    filtered_data_w_jitter <- filtered_data_w_jitter %>%
      ...
      dplyr::select(tidyselect::all_of(c("event", "event_group", "jitter_event_time")))
    ...
    filtered_data <- filtered_data %>% dplyr::left_join(
      filtered_data %>%
        dplyr::select(tidyselect::all_of(c("event","event_group"))) %>%
        dplyr::distinct() %>%
        dplyr::group_by(.data$event) %>%
        dplyr::mutate(number_event_groups = dplyr::n()) %>%
        dplyr::mutate(
          unique_event = dplyr::case_when(
            .data$number_event_groups == 1 ~ .data$event,
            .data$number_event_groups > 1 ~ paste0(.data$event," (",.data$event_group,")")
          )
        ) %>% dplyr::select(tidyselect::all_of(c("event","event_group", "unique_event"))),
