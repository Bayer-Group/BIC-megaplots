...
filter_megaplot_data <- function(
  ...
  if (!is.null(tree)) {
    if (nrow(tree) > 0) {
      ...
      filtered_data <- filtered_data |>
        dplyr::select(
          -tidyselect::all_of(c("event_color", "jittered", "event_id"))
        ) |>
        dplyr::left_join(
          color_data |>
        ...
        for (i in 2:nrow(filtered_data_w_jitter)) {
          if (!is.na(filtered_data_w_jitter$event_group_id[i])) {
            if (filtered_data_w_jitter$jittered[i]) {
              filtered_data_w_jitter$seq_nr[i] <- filtered_data_w_jitter$seq_nr[
                i - 1
              ] +
                1
            } else if (
      ...
      filtered_data_w_jitter <- filtered_data_w_jitter |>
        dplyr::mutate(
          jitter_event_time = seq(
            -0.20,
            0.20,
            length = length(unique(filtered_data_w_jitter$seq_nr))
          )[.data$seq_nr]
        ) |>
        dplyr::select(tidyselect::all_of(c(
          "megaplots_selected_event",
      ...
      filtered_data <- filtered_data |>
        dplyr::ungroup() |>
        dplyr::left_join(
          filtered_data_w_jitter,
          by = c("megaplots_selected_event", "megaplots_selected_event_group")
        ) |>
        dplyr::mutate(
          subjectid_n_jittered = .data$subjectid_n + .data$jitter_event_time
        )
          ...
          filtered_data |>
            ...
            dplyr::mutate(
              unique_event = dplyr::case_when(
                ...
                .data$number_event_groups > 1 ~ paste0(
