...
createFile.sl <- function(path_adsl,
  ...
  adsl <- adsl %>%
    ...
    dplyr::mutate(
      across(all_of(c(display_start_date,display_end_date,relative_day_1,{if(!is.null(trtstdt) & !is.null(trtendt)){c(trtstdt, trtendt)}})),~as.Date(.x)), # Convert specified columns to Date type
