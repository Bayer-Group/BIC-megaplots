...
test_data <- data.frame(
  ...
  event_time = as.POSIXct(c(2, 8, 5, 3, 1, 2)),
  event_time_end = as.POSIXct(c(2, 98, 12, 3, 5, 4))
)

# Tests
test_that("Function returns correct time to first event", {
  result <- calc_time_to_first(
    test_data,
    calc_event = TRUE,
    calc_event_group = FALSE
  )
