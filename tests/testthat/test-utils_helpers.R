test_that("Check if utils_helpers functions work", {
  ...
  test_megaplot_data_raw <- data.frame(
    ...
    subgroup_2 = rep(
      c("Subgroup A", "Subgroup B", "Subgroup C", "Subgroup D"),
      each = 4
    ),
    text_lines = NA,
    event_color = "#ff3322",
  ...
  test_plotly_object <- test_megaplot_data_raw |>
    ...
    plotly::add_segments(
      ...
      line = list(
        color = ifelse(theme == "dark", "#2c3336", "#dedede"),
  ...
  testthat::expect_equal(
    sort(result$name),
    sort(unique(test_megaplot_data_raw$unique_event))
  )
  testthat::expect_equal(
    sort(result$legendgroup),
    test_megaplot_data_raw |>
  ...
  testthat::expect_equal(
    names(result),
    c("type", "fillcolor", "y0", "y1", "yref", "x0", "layer", "x1", "line")
  )
